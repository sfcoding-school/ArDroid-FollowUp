...
public class FeatureScanner {
    ...
    public String doTheWork(Mat mRgba) {
        ...
        if (keypointsImage.size().height >= 2 && frameKeypoints.size().height >= 2) {
            ...
            ArrayList<List<DMatch>> matchesList = new ArrayList<>();
            ...
            ArrayList<Point> objAux = new ArrayList<>();
            ArrayList<Point> frameAux = new ArrayList<>();
            ...
            if (goodMatches.size() >= 15) {
                ...
                for (int i = 0; i < 4; i++) {
                    Point temp = new Point(frameCorners.get(i, 0));
                    if (temp.x > maxX)
                        maxX = (int) temp.x;
                    else if (temp.x < minX)
                        minX = (int) temp.x;
                ...
                if (rect.tl().x + rect.width / 2 < frameSize.width / 3)
                    ...
                    if (rect.height * rect.width < 128 * 128)
                        return "forward";
                    else if (rect.height * rect.width > 132 * 132)
                        return "backward";
                    else
                        return "stop";
                }
